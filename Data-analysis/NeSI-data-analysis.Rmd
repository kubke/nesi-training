...
As there are no direct numbers of researchers in each higher education institution or discipline, these numbers are estimated using the number of funded EPs in the 2012 PBRF round. According to TEC there are 9 universities and a number of other HED institutions that contribute to the PBRF 2012. All these other HED institutions are grouped into one as "all other". 

The fraction of funded EPs per institution or per discipline is used to calculate the number of researchers from the total number of researchers in higher education provided in the government R&D 2016 survey. The R&D number used here is that under the role 'researcher', i.e., excludes technical staff, students, and support staff. This provides an approximation of the total number of academic research staff + research fellows/postdocs.
...
The number of total technician number/FTE in Higher Ed is estimated using the same same approach as above. The number of technicians in the R&D 2016 survey are distributed accross institution/discipline proportionally to the number of funded EPs. 
...
The total number of students in higher education uses the total number of enrolled students in Masters and Phd degrees in 'educationcounts.govt.nz'. The total number is larger than the total number of student researchers in the 2016 R&D survey. 

*ASSUMPTION: that all Masters and PhD students are engaged in research.*
...
total.nesi.learners = sum(reshedall$nesi.learner.total)+sum(rescri$nesi.learner.total)
total.research.workforce = gov.res.number+hed.researcher.number+ hed.student.number+gov.tech.number+ hed.tech.number

cat("Threre are", total.research.workforce, "of researchers (R&D survey, including tech and students), and", total.nesi.learners, "nesi learners over the 4 years reported, which represent", (total.nesi.learners*100/total.research.workforce),"% of the workforce. This does not account for students having been 'learners' more than once.")
